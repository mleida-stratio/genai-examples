...
def mock_chat_messages(mocker: MockerFixture, messages: list[tuple[str, str]]) -> None:
    ...
    def chat_invoke(chat_model, chat_prompt: ChatPromptValue, config) -> BaseMessage:
        input_msg = chat_prompt.messages[-1].content
...
class TestMemoryChain:
    @classmethod
    def setup_class(cls):
        database_dir = "/tmp/var/genai/memory_chain_storage"
        if not os.path.exists(database_dir):
            os.makedirs(database_dir)

    def test_chain(self, mocker: MockerFixture):
        mock_vault(mocker, password={"token": "test-token"})
        msg_input = "which kind of car brakes exists?"
        msg_output = "The main types of car brakes include disc brakes and drum brakes."
        mock_chat_messages(
            mocker,
            messages=[
                (
        ...
        genai_chain = MemoryChain(
            openai_stratio_credential="openai-token",
            chat_temperature=0.5,
            memory_path="memory_chain_storage",
        )
        chain = genai_chain.chain()
        result = chain.invoke(
            {"topic": "cars", "input": "brakes", "question": msg_input},
